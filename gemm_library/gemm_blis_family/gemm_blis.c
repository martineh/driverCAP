...
#include <arm_neon.h>
#include "neon/gemm_blis_neon_fp32.h"
...
void pack_RBX( char orderM, char transM, int mc, int nc, float *M, int ldM, float *Mc, int RR );
void pack_CBX( char orderM, char transM, int mc, int nc, float *M, int ldM, float *Mc, int RR );
void gemm_base_CresidentX( char orderC, int m, int n, int k, 
...
void gemm_blis_B3A2C0( char orderA, char orderB, char orderC,
		       ...
		       float *B, size_t ldB, float beta, float *C, size_t ldC, 
		       float *Ac, float *Bc, size_t MC, size_t NC, size_t KC ) {
  ...
  for ( jc=0; jc<n; jc+=NC ) {
    ...
    for ( pc=0; pc<k; pc+=KC ) {
      ...
      pack_CBX( orderB, transB, kc, nc, Bptr, ldB, Bc, NR);
	...
	//Comment or uncomment for packing or not
        pack_RBX( orderA, transA, mc, kc, Aptr, ldA, Ac, MR);
	...
	//#pragma omp  parallel for private(nr, ir, mr, Cptr) firstprivate (Ac, Bc)   
        for ( jr=0; jr<nc; jr+=NR ) {
	      ...
	      //gemm_base_CresidentX( orderC, mr, nr, kc, alpha, &Ac[ir*kc], MR, &Bc[jr*kc], NR, betaI, Cptr, ldC );
	      gemm_microkernel_Cresident_neon_4x4_fp32( orderC, mr, nr, kc, alpha, &Ac[ir*kc], &Bc[jr*kc], betaI, Cptr, ldC );
	      //gemm_microkernel_Cresident_neon_8x8_fp32( orderC, mr, nr, kc, alpha, &Ac[ir*kc], &Bc[jr*kc], betaI, Cptr, ldC );
